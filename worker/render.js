// Storefront renderer, ported verbatim from bin/build.js (which is now retired).
// Call makeRenderer(products, config) with the catalogue in the exact shape the old
// JSON files had (see worker/db.js), then index() / product(p) return HTML strings
// instead of writing files. The body below is the original build.js unchanged, so
// the pages come out byte-for-byte the same as the static build did.
export function makeRenderer(products, CONFIG) {
...
function buildProduct(p) {
  ...
  return { index: buildIndex, product: buildProduct };
}
